# Create a raw transaction with a transaction fee of 20,000 satoshis 
# The transaction fee should be 20,000 satoshis, then the rest of the amount should be sent to the address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP
# Use the UTXOs from the transaction below
# raw_tx=""TXID=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq -r .txid)
UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq -r '.vout[0].value')
AMOUNT_BTC=$(echo "scale=8; $UTXO_VALUE - 0.00020000" | bc)
bitcoin-cli -regtest createrawtransaction "[{\"txid\":\"$TXID\",\"vout\":0}]" "{\"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP\":$AMOUNT_BTC}"